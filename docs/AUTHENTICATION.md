...
- The login form starts with `GARDEN_ADMIN_USERNAME` and `GARDEN_ADMIN_EMAIL`; after setup, either value can be changed from **Settings → Account**.
...
- Every protected page and write API verifies the token and checks the immutable Supabase user UUID. If no UUID is configured initially, Garden Studio captures it from the authenticated owner the first time account settings are saved.
...
8. Optionally copy the new user's UUID into `GARDEN_ADMIN_USER_ID`. If you skip this, Garden Studio securely records the authenticated owner UUID when you first save Account settings.
...
After the initial sign-in, use **Settings → Account** to update the username, email, or password. Supabase may send a confirmation message when the email changes.
